...
if(isset($_POST['bouton1'])) {
   ...
   if(!empty($_POST['pseudo']) AND !empty($_POST['courriel']) AND !empty($_POST['code1']) AND !empty($_POST['code2']) AND !empty($_POST['codeadmin'])) {
	  if($_POST['codeadmin']=="admin"){
			...
			if(filter_var($courriel, FILTER_VALIDATE_EMAIL)) {
				...
				if($courrielexist == 0) {
					if($code1 == $code2) {
						...
						$tableadmin=$bdd->prepare("INSERT INTO admins(courriel, lvl) VALUES(?, ?)");
						$tableadmin->execute(array($courriel, 1));
...
require "inscription_admin.html";