<?php
function delete($auteur,$commentaire,$commentateur,$dat,$heur)
{
    ...
    //suppression des informations dans la table des commentaire
    $query = $connection->prepare("DELETE FROM Commentaire WHERE pseudo=? AND commentaire=? AND commentateur=? AND dat=? AND heur=?");
    $query->execute(array($auteur,$commentaire,$commentateur,$dat,$heur));
