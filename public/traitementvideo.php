...
$nom=$_POST['nom'];
	...
	$selection = $bdd->query("SELECT * FROM Video WHERE titre= '$nom'");
...
<?php 
 $req=$selection->fetch() 
  ...
  <tr>
    ...
    <td>
    <a href="reservervideo.php?reference=<?php echo($req['reference']);?> & disponibilite= <?php echo($req['disponibilite']);?> & titre=<?php echo($req['titre']);?> ">Reserver<span class="fa fa-check-square-o"></span></a>
    </td>
    <?php
    if($status == "administrateur")
    {
...
?>
   ...
   <div class="col-md-6 col-md-offset-3" style="box-shadow: 0px 0px 8px 8px #D0D0D0;font-family: Consolas;border-radius: 15px;">
    <h1 class="red">VIDEO NON DISPONIBLE</h1></div><?php
