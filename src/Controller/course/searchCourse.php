...
<div id=bite></div>
ville de départ :
<input type=text id=departure oninput="setTimeout(autoSuggestLieu, 150, this, this.value)" list="departureAutoCompl" autocomplete="off" value="<?=isset($_GET['departure']) ? $_GET['departure'] : "" ?>" />
<datalist id="departureAutoCompl"></datalist>
<br/>

ville d'arrivée :
<input type=text id=arrival oninput="setTimeout(autoSuggestLieu, 150, this, this.value)" list="arrivalAutoCompl" autocomplete="off" value="<?=isset($_GET['arrival']) ? $_GET['arrival'] : "" ?>" />
<datalist id="arrivalAutoCompl"></datalist>
<br/>

date :
<input type=date id=date min= <?=date('Y-m-d')?> value="<?=isset($_GET['date']) ? $_GET['date'] : "" ?>" />
<br/>

heure :
<input type=time id=time value="<?=isset($_GET['time']) ? $_GET['time'] : "" ?>" />
<br/>
...
<?=isset($_GET['departure'])&&isset($_GET['arrival'])&&isset($_GET['date'])&&isset($_GET['time']) ? "<script>searchCourse();</script>" : "" ?>